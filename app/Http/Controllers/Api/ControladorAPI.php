    ...
    public function retirar(Request $request)
{
    ...
    try {
        ...
        $cajasParaVaciar = []; // Cambié el nombre de $cajasParaEliminar a $cajasParaVaciar
        ...
        foreach ($cajas as $caja) {
            ...
            if ($caja->kilogramos >= $kilogramosRestantes) {
                ...
                if (($caja->kilogramos - $kilogramosRestantes) == 0) {
                    // En vez de eliminar, vaciar la caja (poner kilogramos a 0)
                    Log::info('Caja vacía. Kilogramos restando a 0.');
                    Caja::where('id', $caja->id)->update(['kilogramos' => 0]);

                    // Eliminar la posición de la caja
        ...
        // No eliminamos las cajas vacías, solo dejamos la lógica de vaciado
        // Las cajas vacías no se eliminan automáticamente ahora

        // Registrar la venta
        $venta = new Venta();
        $venta->Fecha = now();
        $venta->kilogramos = $kilogramosARetirar;
        $venta->precio_total = $kilogramosARetirar * 100; // Asumí un precio por kg de 100, ajusta según sea necesario
